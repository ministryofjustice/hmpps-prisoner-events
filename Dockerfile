...
WORKDIR /builder
COPY hmpps-prisoner-events-${BUILD_NUMBER}.jar app.jar
...
ENTRYPOINT ["java", "-XX:+ExitOnOutOfMemoryError", "-XX:+AlwaysActAsServerClassMachine", "-Doracle.jakarta.jms.useEmulatedXA=false", "-javaagent:agent.jar", "-jar", "app.jar"]
